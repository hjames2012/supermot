  const blockedSites = [
    ...
    document.getElementById('uv-address').addEventListener('keydown', function(event) {
        if (event.key === 'Enter') {
            ...
            if (isBlockedSite(inputValue) || containsBlockedKeyword(inputValue) || containsBlockedKeywordPattern(inputValue)) {
                event.preventDefault(); 
                document.getElementById('uv-error-code').innerText = `You are violating TOS by searching up: ${inputValue}. This has been logged XD`;
                this.blur(); 
            }
        }
    });
    // credit to PLEXILE ARCADE love you willo
